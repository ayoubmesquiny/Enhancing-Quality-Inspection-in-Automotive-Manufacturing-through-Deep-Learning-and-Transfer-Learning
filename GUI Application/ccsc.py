...
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QPushButton, QVBoxLayout,QMessageBox,QDesktopWidget,QSizePolicy
...
class VideoCaptureApp(QMainWindow):
    ...
    def load_model():
        ...
        model_path = os.path.join(script_dir, 'model/DenseNet121.h5')
    ...
    def update_frame(self):
        ...
        # ...reshape it into a numpy array...
        frame = np.reshape(array, (height.value, width.value, bytes_per_pixel))
        self.capture = frame

        # Increase brightness
        brightness_factor = 1.3  # Adjust the factor as needed
        frame = cv2.convertScaleAbs(self.capture, alpha=brightness_factor)
    ...
    def save_frame(self):
        ...
        # Increase brightness (optional, if you want to save the image with increased brightness)
        brightness_factor = 1.3
        frame = cv2.convertScaleAbs(frame, alpha=brightness_factor)
        ...
        # Save the image without the blue channel
        ...
        if predicted_class_probability >= threshold:
            ...
            self.inspect_and_show_message("Prediction: OK",True)
            ...
            with open(file_path, 'w') as file:
                file.write(data)
            self.inspect_and_show_message("Prediction: NOK",False)


    def inspect_and_show_message(self, message, is_information):
        self.display_info = Message(message, "#32cd32" if is_information else "#FF2400", "#355e3b" if is_information else "#7C0A02")
